...
def norm_g(g,sid):
    ...
    for et in g.edge_types:
        ...
        if ea is not None and ea.shape[0]>0:
            ...
            if ea.shape[1]>=1: ea[:,0]=0.0  # ABLATION: zero t_hours
...
print("Ablation: NO TIME IN EDGE FEATURES (t_hours zeroed)")
...
print("Normalizing (t_hours zeroed, reverse edges added)...")
...
print(f"Edge types: {len(edge_types)} (should be 8)")
for et in sample_g.edge_types:
    ea=sample_g[et].edge_attr
    if ea is not None and ea.shape[0]>0:
        print(f"col0 t_hours (must be 0.0): {ea[:3,0].tolist()}")
        print(f"col1 value   (nonzero OK):  {ea[:3,1].tolist()}")
        break
...
sampler=torch.utils.data.WeightedRandomSampler(torch.tensor(weights,dtype=torch.float),len(train_graphs),replacement=True)
...
def make_conv(h,d,ets):
    return HeteroConv({et:GATv2Conv((-1,-1),h//4,heads=4,edge_dim=2,dropout=d,add_self_loops=False,concat=True) for et in ets},aggr='sum')

class CHIRPNoTime(nn.Module):
    def __init__(self,hidden=192,dropout=0.25,node_types=None,edge_types=None):
        ...
        self.enc=nn.ModuleDict({nt:nn.Sequential(nn.Linear(fd.get(nt,4),hidden),nn.LayerNorm(hidden),nn.ReLU()) for nt in node_types})
    ...
    def forward(self,data):
        ...
        for et in data.edge_types:
            ...
            if ea is not None and ea.shape[0]>0:
                if ea.shape[1]<2: ea=torch.cat([ea,torch.zeros(ea.shape[0],2-ea.shape[1],device=ea.device)],dim=1)
                ea_d[et]=ea[:,:2].float()
...
model=CHIRPNoTime(hidden=192,dropout=0.25,node_types=node_types,edge_types=edge_types).to(device)
...
full_aurocs={42:0.8514,43:0.8463,44:0.8324,45:0.8429,46:0.8517}  # from verified chirp_clean_seed*.log
...
print(f"\n{'='*55}\nAblation: NO TIME IN EDGE FEATURES | Seed {SEED}\n{'='*55}")
best_auc,patience=0,0
for epoch in range(1,51):
    model.train(); total_loss,count,skipped=0,0,0
    for b in train_loader:
        try:
            b=b.to(device); optimizer.zero_grad()
            mort_out,los_out=model(b)
            y_mort=b.y.to(device).flatten(); mort_out=mort_out.flatten()
            ...
            optimizer.step(); total_loss+=loss_mort.item(); count+=1
    ...
    if np.isfinite(val_auc) and val_auc>best_auc:
        ...
        torch.save(model.state_dict(),MODEL_DIR/f'ablation_no_time_seed{SEED}.pt')
        print(f"           *** Best: {best_auc:.4f} ***")
    else:
        patience+=1
        if patience>=15: print(f"Early stopping at epoch {epoch}"); break

model.load_state_dict(torch.load(MODEL_DIR/f'ablation_no_time_seed{SEED}.pt'))
test_auc,test_ap=evaluate(test_loader)
full_auc=full_aurocs.get(SEED,0.8951)
print(f"\nBest Val AUROC: {best_auc:.4f}")
print(f"Test AUROC:  {test_auc:.4f} | AUPRC: {test_ap:.4f}")
print(f"Full CHIRP-Net seed {SEED}: {full_auc:.4f}")
print(f"Delta (time-attentive edges): +{full_auc-test_auc:.4f}")
print(f"\nThis is your CHIRP no-time ablation result!")
pd.DataFrame([{'ablation':'no_time_edge_features','seed':SEED,'test_auroc':test_auc,'test_auprc':test_ap,'full_auroc':full_auc,'delta':full_auc-test_auc}]).to_csv(f'/root/results/ablation_no_time_seed{SEED}.csv',index=False)
print(f"Saved: /root/results/ablation_no_time_seed{SEED}.csv")
