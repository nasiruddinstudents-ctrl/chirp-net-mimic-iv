"""
Ablation: CHIRP-Net without reverse edges
...
print("Preloading raw graphs (no reverse edges ablation)...")
graphs_ram = {}
...
def norm_g(g, sid):
    ...
    for et in g.edge_types:
        ...
        if ea is not None and ea.shape[0]>0:
            ...
            g[et].edge_attr = ea
    # NO ToUndirected — this is the ablation
...
print("Normalizing (no reverse edges added)...")
...
print(f"Edge types: {len(edge_types)} (should be 4, not 8 — confirms no reverse edges)")
...
class CHIRPNoReverse(nn.Module):
...
model = CHIRPNoReverse(hidden=192, dropout=0.25,
                       node_types=node_types, edge_types=edge_types).to(device)
with torch.no_grad():
...
print(f"\n{'='*55}")
print(f"Ablation: NO REVERSE EDGES | Seed {SEED}")
print(f"{'='*55}")
...
for epoch in range(1, 51):
    ...
    if np.isfinite(val_auc) and val_auc > best_auc:
        best_auc = val_auc
        patience = 0
        torch.save(model.state_dict(), MODEL_DIR/f'ablation_no_reverse_seed{SEED}.pt')
...
model.load_state_dict(torch.load(MODEL_DIR/f'ablation_no_reverse_seed{SEED}.pt'))
...
print(f"Delta (reverse edges): +{full_auc-test_auc:.4f}")

pd.DataFrame([{
    'ablation': 'no_reverse_edges',
    'seed': SEED,
    'test_auroc': test_auc,
    'test_auprc': test_ap,
    'full_auroc': full_auc,
    'delta': full_auc - test_auc,
}]).to_csv(f'/root/results/ablation_no_reverse_seed{SEED}.csv', index=False)
print(f"Saved: /root/results/ablation_no_reverse_seed{SEED}.csv")
print(f"\nThis is your CHIRP ablation result!")
